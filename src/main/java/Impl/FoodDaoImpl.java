...
public class FoodDaoImpl implements FoodDao{
	...
	public boolean addFood(Food f) 
	{
		try
		{
			...
			String sql= "insert into Food_ctol18(foodName,foodType,foodCategory,foodDesc,foodPrice,image) values(?,?,?,?,?,?)";
	...
	public boolean updateFood(Food f) 
	{
		try 
		{
			...
			String sql="update Food_ctol18 set foodName=?,foodType=?,foodCategory=?,foodDesc=?,foodPrice=?,image=? where foodId=?";
	...
	public List<Food> searchFoodByName(String foodName) 
	{
		...
		{
			Connection con=DButility.getConnect();
			String sql="select *from Food_ctol18 where foodName like ? ";
			PreparedStatement ps=con.prepareStatement(sql);
	...
	public List<Food> searchFoodByCategory(String Category) 
	{
		...
		{
			Connection con=DButility.getConnect();
			String sql="select *from Food_ctol18 where foodCategory=?";
			PreparedStatement ps=con.prepareStatement(sql);
